...
bool ProcessState::becomeContextManager(context_check_func checkFunc, void* userData) {
    if (!mManagesContexts) {
        ...
        //  is used to set the maximum number of allowed Binder threads that driver can request user space to spawn
        if (result == -1) {
            if (result == 0) {
...
void ProcessState::spawnPooledThread(bool isMain) {
    // The isMain indicates whether this thread joins thread pool voluntarily.
    if (mThreadPoolStarted) {
...
ProcessState::ProcessState()
    ...
    if (mDriverFD >= 0) {
        ...
        // setup a kernel buffer mapping for current process
